...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	var (
		...
		carpoolTermID             sql.NullInt64
		carpoolCycleID            sql.NullInt64
		carpoolAdmittedAt         sql.NullTime
		inputTokens               int
		outputTokens              int
	...
	if carpoolTermID.Valid {
		value := carpoolTermID.Int64
		log.CarpoolTermID = &value
	}
	if carpoolCycleID.Valid {
		value := carpoolCycleID.Int64
		log.CarpoolCycleID = &value
	}
	if carpoolAdmittedAt.Valid {
		value := carpoolAdmittedAt.Time
		log.CarpoolAdmittedAt = &value
	}
	if durationMs.Valid {
