...
func TestUsageLogRepositoryCreateSyncRequestTypeAndLegacyFields(t *testing.T) {
	...
	mock.ExpectQuery("INSERT INTO usage_logs").
		WithArgs(
			...
			sqlmock.AnyArg(), // carpool_term_id
			sqlmock.AnyArg(), // carpool_cycle_id
			sqlmock.AnyArg(), // carpool_admitted_at
			log.InputTokens,
			log.OutputTokens,
...
func TestScanUsageLogRequestTypeAndLegacyFallback(t *testing.T) {
	t.Run("image_size_metadata_is_scanned", func(t *testing.T) {
		...
		log, err := scanUsageLog(usageLogScannerStub{values: []any{
			...
			sql.NullInt64{},
			sql.NullInt64{},
			sql.NullInt64{Valid: true, Int64: 41},
			sql.NullInt64{Valid: true, Int64: 42},
			sql.NullTime{Valid: true, Time: now.Add(-time.Minute)},
			0, 0, 0, 0, 0, 0,
		...
		require.Equal(t, int64(41), *log.CarpoolTermID)
		require.Equal(t, int64(42), *log.CarpoolCycleID)
		require.Equal(t, now.Add(-time.Minute), *log.CarpoolAdmittedAt)
	})
	...
	t.Run("request_type_ws_v2_overrides_legacy", func(t *testing.T) {
		...
		log, err := scanUsageLog(usageLogScannerStub{values: []any{
			...
			sql.NullInt64{},   // carpool_term_id
			sql.NullInt64{},   // carpool_cycle_id
			sql.NullTime{},    // carpool_admitted_at
			1,                 // input_tokens
			2,                 // output_tokens
