...
func (h *OpenAIGatewayHandler) SeedanceTasks(c *gin.Context) {
	...
	// Async video tasks do not persist the create-time carpool admission or
	// cycle. Reject before any upstream work rather than leaving usage unsettled.
	if key.Group.IsCarpoolType() {
		h.errorResponse(c, http.StatusForbidden, "CARPOOL_SEEDANCE_UNSUPPORTED", "Seedance is not supported for carpool billing")
		return
	}
