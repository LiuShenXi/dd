...
type UsageLog struct {
	...
	// Internal carpool billing term snapshot; excluded from member DTOs
	CarpoolTermID *int64 `json:"carpool_term_id,omitempty"`
	// Internal carpool billing cycle snapshot; excluded from member DTOs
	CarpoolCycleID *int64 `json:"carpool_cycle_id,omitempty"`
	// Final upstream admission time for immutable carpool attribution
...
func (*UsageLog) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case usagelog.FieldCarpoolAdmittedAt, usagelog.FieldCreatedAt:
...
func (_m *UsageLog) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case usagelog.FieldCarpoolTermID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field carpool_term_id", values[i])
			} else if value.Valid {
				_m.CarpoolTermID = new(int64)
				*_m.CarpoolTermID = value.Int64
			}
		case usagelog.FieldCarpoolCycleID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field carpool_cycle_id", values[i])
			} else if value.Valid {
				_m.CarpoolCycleID = new(int64)
				*_m.CarpoolCycleID = value.Int64
			}
		case usagelog.FieldCarpoolAdmittedAt:
			if value, ok := values[i].(*sql.NullTime); !ok {
				return fmt.Errorf("unexpected type %T for field carpool_admitted_at", values[i])
			} else if value.Valid {
				_m.CarpoolAdmittedAt = new(time.Time)
				*_m.CarpoolAdmittedAt = value.Time
			}
...
func (_m *UsageLog) String() string {
	...
	builder.WriteString(", ")
	if v := _m.CarpoolTermID; v != nil {
		builder.WriteString("carpool_term_id=")
		builder.WriteString(fmt.Sprintf("%v", *v))
	}
	builder.WriteString(", ")
	if v := _m.CarpoolCycleID; v != nil {
		builder.WriteString("carpool_cycle_id=")
		builder.WriteString(fmt.Sprintf("%v", *v))
	}
	builder.WriteString(", ")
	if v := _m.CarpoolAdmittedAt; v != nil {
		builder.WriteString("carpool_admitted_at=")
