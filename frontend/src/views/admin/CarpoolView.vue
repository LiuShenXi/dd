<template>
  <AppLayout>
    <div class="space-y-5">
      ...
      <div v-if="activeTab !== 'plans' && activeTab !== 'resets'" class="flex flex-wrap items-end gap-3">
      ...
      <div v-else-if="activeTab === 'resets'" class="space-y-6">
        <div class="flex flex-wrap gap-2"><button data-testid="official-reset-open" class="btn btn-danger" @click="openOfficialReset"><Icon name="refresh" size="sm" />{{ t('admin.carpool.reset.official.action') }}</button><button class="btn btn-primary" @click="openResetRegister"><Icon name="plus" size="sm" />{{ t('admin.carpool.reset.register') }}</button><button class="btn btn-secondary" :disabled="scanning" @click="scanObservations"><Icon name="refresh" size="sm" />{{ t('admin.carpool.reset.scan') }}</button></div>
    ...
    <BaseDialog :show="showOfficialReset" :title="t('admin.carpool.reset.official.title')" @close="closeOfficialReset"><form data-testid="official-reset-form" class="space-y-4" @submit.prevent="submitOfficialReset"><p class="text-sm text-gray-700 dark:text-gray-200">{{ t('admin.carpool.reset.official.description') }}</p><p class="text-sm font-medium text-amber-700 dark:text-amber-300">{{ t('admin.carpool.reset.official.expiry') }}</p><p v-if="officialResetError" class="text-sm text-red-600">{{ officialResetError }}</p><div class="flex justify-end gap-2"><button type="button" class="btn btn-secondary" :disabled="officialResetSubmitting" @click="closeOfficialReset">{{ t('common.cancel') }}</button><button data-testid="official-reset-confirm" class="btn btn-danger" :disabled="officialResetSubmitting">{{ t('admin.carpool.reset.official.confirm') }}</button></div></form></BaseDialog>
...
import { adminNextNaturalResetAt, formatCarpoolAmount } from '@/utils/carpool'
...
type Tab = 'terms' | 'cycles' | 'ledger' | 'exceptions' | 'resets' | 'plans'
const { t } = useI18n(); const router = useRouter(); const appStore = useAppStore()
const activeTab = ref<Tab>('terms'); const loading = ref(false); const loadError = ref(false); const submitting = ref(false)
const terms = ref<CarpoolAdminTerm[]>([]); const cycles = ref<CarpoolAdminCycle[]>([]); const ledger = ref<CarpoolLedgerEntry[]>([]); const billingExceptions = ref<CarpoolBillingException[]>([]); const plans = ref<CarpoolPlan[]>([]); const resetBatches = ref<CarpoolResetBatch[]>([]); const resetObservations = ref<CarpoolResetObservation[]>([])
const filters = reactive<{ user_id: number | null; status: string }>({ user_id: null, status: '' })
const paginations = reactive<Record<Exclude<Tab, 'plans'>, { page: number; page_size: number; total: number }>>({ terms: { page: 1, page_size: 20, total: 0 }, cycles: { page: 1, page_size: 20, total: 0 }, ledger: { page: 1, page_size: 20, total: 0 }, exceptions: { page: 1, page_size: 20, total: 0 }, resets: { page: 1, page_size: 20, total: 0 } })
...
const tabs = computed(() => (['terms', 'cycles', 'ledger', 'exceptions', 'resets', 'plans'] as Tab[]).map((key) => ({ key, label: t(`admin.carpool.tabs.${key}`) })))
...
function userLabel(record: { user_id: number; user_email?: string }) { return record.user_email || `#${record.user_id}` }
...
const renewTarget = ref<CarpoolAdminTerm | null>(null); const renewForm = reactive<{ plan_id: number | null; notes: string }>({ plan_id: null, notes: '' }); let renewKey = ''; let renewRequestFingerprint = ''
const renewPlanOptions = computed(() => {
  const snapshot = renewTarget.value?.plan_snapshot
  if (!snapshot || !(snapshot.weekly_quota_customized || snapshot.duration_customized) || !plans.value.some((plan) => plan.enabled && plan.code === snapshot.code)) return planOptions.value
  return [{ value: 0, label: t('admin.carpool.renewal.keepCurrentTerms', { quota: money(snapshot.weekly_quota_usd), days: snapshot.duration_days }) }, ...planOptions.value]
})
...
function openRenew(term: CarpoolAdminTerm) { beginModalAction(); renewTarget.value = term; renewForm.plan_id = renewPlanOptions.value.some((option) => option.value === 0) ? 0 : plans.value.find((plan) => plan.enabled && plan.code === term.plan_snapshot.code)?.plan_id ?? null; renewForm.notes = ''; renewKey = key(); renewRequestFingerprint = '' }
...
async function submitRenew() {
  ...
  if (!target || planID === null || submitting.value) return
  const requestGeneration = modalGeneration; const request = { plan_id: planID, notes: renewForm.notes || null, payment: null }
  const fingerprint = JSON.stringify(request)
  if (renewRequestFingerprint && renewRequestFingerprint !== fingerprint) renewKey = key()
  renewRequestFingerprint = fingerprint
  const requestKey = renewKey
...
const showOfficialReset = ref(false); const officialResetSubmitting = ref(false); const officialResetError = ref(''); let officialResetKey = ''
function openOfficialReset() { if (officialResetSubmitting.value) return; showOfficialReset.value = true; officialResetError.value = ''; officialResetKey ||= key() }
function closeOfficialReset() { if (officialResetSubmitting.value) return; showOfficialReset.value = false; officialResetError.value = '' }
async function submitOfficialReset() {
  if (!showOfficialReset.value || officialResetSubmitting.value) return
  officialResetSubmitting.value = true; officialResetError.value = ''
  try {
    const batch = await adminAPI.carpool.executeOfficialReset(officialResetKey)
    officialResetKey = ''
    showOfficialReset.value = false
    appStore.showSuccess(t('admin.carpool.reset.official.success', { count: batch.target_count ?? 0 }))
    await loadActive()
  } catch (error) {
    officialResetError.value = errorMessage(error)
  } finally {
    officialResetSubmitting.value = false
  }
}
