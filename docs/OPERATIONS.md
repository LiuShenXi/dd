# Sub2API 2026-09-10 生产环境运维历史记录

> **2026-09-19 01:42 更新：** Codex 票据功能已按用户要求开启，采集使用现有 `US-static-car8` 代理；二号车 Astra/Sol 的 292 位票据已获取且管理端 ready。未重启应用。验证范围与已知限制见 [票据配置记录](CODEX_TICKET_20260919.md)。

> **2026-09-19 更新：当前生产在搬瓦工 `bwh` 的 `/home/linuxuser/apps/sub2api`，活动容器为 `sub2api-blue`，版本 `0.2.6-carpool-20260919`，镜像 `sub2api:carpool-compatible-0.2.6-a8370c552`。** 01:19:55（北京时间）完成公网与业务就绪核验，原 green 已停止并保留。详见 [生产发布记录](RELEASE_0_2_6_20260919.md)。下文服务器、槽位、路由均为 9 月 10 日历史快照，不能直接作为当前操作目标；操作前必须再次读取实时状态。

基线核验时间：**2026-09-10 17:39–17:42，北京时间（Asia/Shanghai）**。**2026-09-11 新增账号「8」的美国代理出口，最新增量见第 11 节；该节覆盖下文基线中的账号数量与代理绑定快照。** 本手册面向后续接手运维的人和自动化任务。动态配置、额度、余额、活动槽位必须在操作前重新读取。
...
## 11. 2026-09-11 账号「8」新增固定代理出口

用户指定邮箱 `a0919177518@gmail.com`。实际对应 **账号 ID 9，显示名称「8」**；数据库 ID 8 的「八号车」是另一个邮箱，不能按数字误绑。本次只修改 ID 9 的代理绑定。

| 项目 | 新配置 |
|---|---|
| 代理记录 | ID 3，`US-static-car8` |
| 协议 / 入口 | HTTP `168.158.30.112:43479` |
| 实测公网出口 | `168.158.30.112` |
| 地理查询 | IPinfo：Mesa / Arizona / US；Cloudflare trace：`loc=US` |
| ASN 查询 | `AS20012 Interworks Networking Services` |
| 状态 / 回退 | active；`fallback_mode=none`；无 backup proxy |

这条代理是美国出口，与之前两条台湾出口不同。供应商标称的“静态家宽”属性没有独立认证；本文不承诺住宅性质、独享性、长期固定或免风控。

绑定前从新服务器验证：代理认证和 HTTPS 成功，出口 IP 查询约 0.448 秒；使用账号 9 的现有 OAuth 向正常上游发送最小 `gpt-6-astra` 请求，HTTP 200，约 4.204 秒收到完整 `response.completed`，输出 `OK`，无流内错误。这是账号经指定代理的上游探测，未把它表述为一次公开网关计费验收。

绑定事务锁定账号并核对邮箱及原 `proxy_id=NULL`，新增代理记录、修改账号绑定，并投递 `account_changed`（事件 ID 53961，分组 2）。随后核验 Redis `sched:acc:9` 为代理 ID 3、水位 53962，并确认应用进程对 `168.158.30.112:43479` 的实际 ESTABLISHED 连接。未重启应用，未改动余额、额度、优先级、可调度开关或分组。

变更后读取的未删除账号中，1「pro 20x」、7「七号车」、9「8」可调度，分别绑定代理 1、2、3；2、5、6 不可调度且没有代理。后续开通或重新启用账号时必须重新读取，不能沿用 9 月 10 日“只有两台可调度”的历史状态。

新机私有审计目录 `/root/sub2api-ops-20260911/`，含 `car8-binding-before.json`、`car8-binding-result.jsonl`、`rollback-car8-binding.sql` 及私有代理认证文件。回退脚本未执行；它核对账号邮箱与当前代理后解除绑定，会恢复默认出口，不应当作自动故障恢复措施。认证资料未写入此仓库。
