...
> **2026-09-18 更新：本页下述服务器、活动槽位和路由信息是 9 月 10 日的历史快照，不能直接作为当前操作目标。** 本次只读核验确认当前 Sub2API 在搬瓦工 `bwh` 的 `/home/linuxuser/apps/sub2api`，活动容器为 `sub2api-green`，镜像为 `sub2api:carpool-imagefix-20260915-gpt55`。当前候选版本与兼容升级说明见 [0.2.6 升级记录](UPGRADE_0_2_6.md)。任何发布前应再次读取实时状态。
